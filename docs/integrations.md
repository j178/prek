...
COPY --from=ghcr.io/j178/prek:0.5.1 /prek /usr/local/bin/prek
```

If you prefer, you can also run the distroless image directly:

```bash
docker run --rm ghcr.io/j178/prek:0.5.1 --version
```
...
```bash
docker run --rm ghcr.io/j178/prek:0.5.1-alpine --version
```
...
!!! tip
    ...
    Use a specific version tag (e.g., `ghcr.io/j178/prek:0.5.1`) or image
