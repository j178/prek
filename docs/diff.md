...
- `prek` supports `repo: self` for projects that publish hooks via `.pre-commit-hooks.yaml` to consume their own hooks (using ephemeral per-run hook environments).
