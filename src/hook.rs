...
impl InstallInfo {
    ...
    pub(crate) fn from_env_path(path: &Path) -> Result<Self> {
        let mut file = fs_err::File::open(path.join(HOOK_MARKER))?;
        let info: InstallInfo = serde_json::from_reader(&mut file)?;
