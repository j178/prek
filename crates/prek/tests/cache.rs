...
#[test]
fn cache_gc_keeps_self_repo_hook_env_with_explicit_relative_config_path() -> anyhow::Result<()> {
    let context = TestContext::new();
    context.init_project();
    ...
    // Run with an explicit relative --config path to exercise path normalization.
    cmd_snapshot!(
        context.filters(),
        context
            .command()
            .args(["run", "--config", ".pre-commit-config.yaml"]),
        @r"
    success: true
    exit_code: 0
    ----- stdout -----
...
#[test]
fn cache_gc_keeps_self_repo_hook_env_with_subdir_config_path() -> anyhow::Result<()> {
    let context = TestContext::new();
    context.init_project();

    let cwd = context.work_dir();
    let subdir = cwd.child("sub");
    subdir.create_dir_all()?;

    // Manifest stays at git root.
    cwd.child(".pre-commit-hooks.yaml")
        .write_str(indoc::indoc! {r#"
    - id: self-python
      name: Self Python Hook
    ...
    // Config lives in a subdirectory and references repo:self.
    subdir
    ...
    cmd_snapshot!(
        ...
        context
            .command()
            .args(["run", "--config", "sub/.pre-commit-config.yaml"]),
        @r"
    success: true
    exit_code: 0
    ----- stdout -----
...
#[test]
fn cache_gc_prunes_stale_root_self_repo_env_when_only_subdir_config_tracked() -> anyhow::Result<()>
{
    let context = TestContext::new();
    context.init_project();
    ...
    "#})?;
    cwd.child("setup.py")
        .write_str("from setuptools import setup; setup(name='rootpkg', version='0.0.1')")?;
    context.write_pre_commit_config(indoc::indoc! {r"
        repos:
          - repo: self
            hooks:
    ...
    // Subdir manifest + config with same hook id.
    subdir
        .child(".pre-commit-hooks.yaml")
        .write_str(indoc::indoc! {r#"
    - id: shared
      name: Sub Hook
      entry: python -c "print('sub')"
      language: python
    "#})?;
    subdir
        .child("setup.py")
        .write_str("from setuptools import setup; setup(name='subpkg', version='0.0.1')")?;
    subdir
        .child(".pre-commit-config.yaml")
        .write_str(indoc::indoc! {r"
        repos:
    ...
    cmd_snapshot!(
        context.filters(),
        context.command().args(["run", "--config", ".pre-commit-config.yaml"]),
        @r"
    success: true
    exit_code: 0
    ----- stdout -----
    ...
    // Drop root config; only subdir config should remain tracked after GC.
    fs_err::remove_file(cwd.child(".pre-commit-config.yaml").path())?;
    context.git_add(".");

    // Install subdir env and ensure subdir config is tracked.
    cmd_snapshot!(
        context.filters(),
        context.command().arg("run").current_dir(subdir.path()),
        @r"
    success: true
    exit_code: 0
    ----- stdout -----
    ...
    let home = context.home_dir();
    let hooks_dir = home.child("hooks");
    let root_dep = fs_err::canonicalize(cwd.path())
        .unwrap_or_else(|_| cwd.path().to_path_buf())
        .to_string_lossy()
        .to_string();
    let sub_dep = fs_err::canonicalize(subdir.path())
        .unwrap_or_else(|_| subdir.path().to_path_buf())
        .to_string_lossy()
        .to_string();

    let mut root_envs = Vec::new();
    let mut sub_envs = Vec::new();
    for entry in fs_err::read_dir(hooks_dir.path())? {
        let entry = entry?;
        if !entry.file_type()?.is_dir() {
            continue;
        ...
        let marker_path = env_dir.join(".prek-hook.json");
        if !marker_path.is_file() {
            continue;
        }
        let marker = fs_err::read_to_string(&marker_path)?;
        let marker_json: serde_json::Value = serde_json::from_str(&marker)?;
        let deps = marker_json["dependencies"]
            .as_array()
            .cloned()
            .unwrap_or_default();
        let dep_strings: Vec<String> = deps
            ...
            .collect();
        if dep_strings.iter().any(|d| d == &root_dep) {
            root_envs.push(env_dir.clone());
        }
        if dep_strings.iter().any(|d| d == &sub_dep) {
            sub_envs.push(env_dir);
        }
    }
    assert!(!root_envs.is_empty(), "expected at least one root self env");
    assert!(
        !sub_envs.is_empty(),
        "expected at least one subdir self env"
    );
    ...
    for env in root_envs {
        assert!(
            !env.exists(),
            "expected stale root self env to be pruned: {}",
            env.display()
        );
    }
    for env in sub_envs {
        assert!(
            env.exists(),
            "expected active subdir self env to remain: {}",
            env.display()
        );
    }
...
fn cache_gc_drops_missing_tracked_config() -> anyhow::Result<()> {
    ...
    let tracked: Vec<serde_json::Value> = serde_json::from_str(&content)?;
