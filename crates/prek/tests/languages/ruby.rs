...
/// Regression test: when prek auto-downloads Ruby (rv-ruby) and the parent
/// process has no `gem` (and no `ruby`) on `PATH`, hook install must still
/// succeed.
///
/// Without prepending the resolved Ruby's bin dir to `PATH` for the
/// `ruby -S gem ...` invocations in `gem.rs`, `prek install-hooks` errors
/// with:
///
///     /<PREK_HOME>/tools/ruby/<version>/bin/ruby: No such file or directory -- gem (LoadError)
///
/// Exercises both `build_gemspec` (gemspec in repo) and `resolve_gems` /
/// `install_single_gem` (`additional_dependencies`).
#[test]
#[cfg(not(target_os = "windows"))]
fn auto_download_gem_install_without_gem_on_path() -> anyhow::Result<()> {
    ...
    if !EnvVars::is_set(EnvVars::CI) {
        // Skip when not running in CI: local environments may have
        // unexpected Ruby versions installed and may not have network
        // access to download rv-ruby.
        return Ok(());
    }
    ...
    // A gemspec in the local repo to exercise `build_gemspec`.
    context
        .work_dir()
        .child("test_gem.gemspec")
        .write_str(indoc::indoc! {r#"
            Gem::Specification.new do |spec|
              ...
              spec.authors       = ["Test"]
              spec.email         = ["test@example.com"]
              spec.summary       = "Test gem for prek regression test"
              spec.files         = []
              spec.require_paths = ["lib"]
            end
        "#})?;

    // `additional_dependencies` exercises `gem install --explain` and
    // `gem install`. Pin `language_version` to a version that prek can
    // auto-download via rv-ruby.
    context.write_pre_commit_config(indoc::indoc! {r#"
        repos:
          - repo: local
            hooks:
              - id: ruby-with-deps
                name: ruby-with-deps
                language: ruby
                entry: ruby -e "require 'webrick'; puts WEBrick::VERSION"
                language_version: '3.2.9'
                additional_dependencies: ["webrick"]
                pass_filenames: false
                always_run: true
    ...
    // Scrub `ruby` and `gem` from PATH so the test would have failed before
    // the fix:
    // - removing `ruby` ensures prek must auto-download via rv-ruby
    //   (no system Ruby is available)
    // - removing `gem` ensures `ruby -S gem ...` can only resolve `gem`
    //   if the resolved Ruby's bin dir is prepended to `PATH` (the bug
    //   fix being tested)
    ...
    cmd_snapshot!(filters, context.run().arg("-v").env(EnvVars::PATH, &new_path), @r"
    success: true
    exit_code: 0
    ----- stdout -----
    ruby-with-deps...........................................................Passed
    - hook id: ruby-with-deps
    - duration: [TIME]
    ...
    // Verify Ruby was actually auto-downloaded (i.e. the test really did
    // exercise the rv-ruby code path, not a system Ruby).
    let installed_versions = ruby_dir
        .read_dir()?
        .flatten()
        .filter_map(|d| {
    ...
    assert!(
        installed_versions.iter().any(|v| v.starts_with("3.2.9")),
        "Expected Ruby 3.2.9 to be auto-downloaded, but found: {installed_versions:?}"
    );
