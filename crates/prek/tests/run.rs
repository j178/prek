...
/// `git commit` exports `GIT_INDEX_FILE` (and, from a linked worktree, absolute
/// `GIT_DIR`/`GIT_INDEX_FILE` paths) to hook processes. Internal git commands
/// against the hook repo cache must not honor them, or a clone checkout
/// replaces the user's index with the hook repo's tree.
#[test]
fn leaked_git_index_file_is_not_written_by_repo_clone() -> Result<()> {
    let context = TestContext::new();
    context.init_project();

    let hook_repo = context.home_dir().child("sentinel-hook-repo");
    hook_repo.create_dir_all()?;
    ...
    hook_repo
        .child("sentinel.txt")
        .write_str("only exists in the hook repo\n")?;
