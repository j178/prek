...
/// Install hooks to the directory specified by `core.hooksPath` (relative path).
#[test]
fn install_with_hooks_path() {
    let context = TestContext::new();
    context.init_project();

    // Use a relative path to exercise resolution against git common dir.
    // Relative paths are resolved against .git/, so "custom-hooks" becomes ".git/custom-hooks".
    ...
    exit_code: 0
    ----- stdout -----
    prek installed at `.git/custom-hooks/pre-commit`
    ...
    insta::with_settings!(
        { filters => context.filters() },
        {
            assert_snapshot!(context.read(".git/custom-hooks/pre-commit"), @r#"
            #!/bin/sh
            # File generated by prek: https://github.com/j178/prek
            # ID: 182c10f181da4464a3eec51b83331688
...
/// Running `prek install` twice with `core.hooksPath` succeeds (idempotent).
#[test]
fn install_with_hooks_path_idempotent() {
    let context = TestContext::new();
    context.init_project();
...
#[cfg(unix)]
#[test]
fn install_with_hooks_path_readonly() -> anyhow::Result<()> {
    use std::os::unix::fs::PermissionsExt;

    let context = TestContext::new();
    context.init_project();

    let hooks_dir = context.work_dir().child(".git/readonly-hooks");
    hooks_dir.create_dir_all()?;
    fs_err::set_permissions(&*hooks_dir, std::fs::Permissions::from_mode(0o555))?;
    ...
    ----- stderr -----
    error: Cannot install hooks: `core.hooksPath` is set to `.git/readonly-hooks` which is not writable.
    hint: Make sure this directory is writable, or change `core.hooksPath` to point to a writable directory.
    "#);

    // Restore permissions for cleanup.
    fs_err::set_permissions(&*hooks_dir, std::fs::Permissions::from_mode(0o755))?;

    Ok(())
}

/// Install hooks to an absolute `core.hooksPath` (skips git common dir resolution).
#[test]
fn install_with_hooks_path_absolute() {
    let context = TestContext::new();
    context.init_project();

    let hooks_dir = context.work_dir().child("absolute-hooks");

    // Use an absolute path to exercise the non-resolution code path.
    git_cmd(context.work_dir())
        .arg("config")
        .arg("core.hooksPath")
        .arg(&*hooks_dir)
    ...
    exit_code: 0
    ----- stdout -----
    prek installed at `absolute-hooks/pre-commit`
    ...
    insta::with_settings!(
        { filters => context.filters() },
        {
            assert_snapshot!(context.read("absolute-hooks/pre-commit"), @r#"
            #!/bin/sh
            # File generated by prek: https://github.com/j178/prek
            # ID: 182c10f181da4464a3eec51b83331688
...
/// Install hooks when `core.hooksPath` points to a non-existent directory (gets created).
#[test]
fn install_with_hooks_path_nonexistent() {
    let context = TestContext::new();
    context.init_project();

    // Use a relative path that does not exist yet.
    git_cmd(context.work_dir())
        .arg("config")
        .arg("core.hooksPath")
        .arg("new-hooks-dir")
    ...
    assert!(!context.work_dir().join(".git/new-hooks-dir").exists());

    cmd_snapshot!(context.filters(), context.install(), @r#"
    success: true
    exit_code: 0
    ----- stdout -----
    prek installed at `.git/new-hooks-dir/pre-commit`
    ...
    assert!(
        context
            .work_dir()
            .join(".git/new-hooks-dir/pre-commit")
            .exists()
    );
}
