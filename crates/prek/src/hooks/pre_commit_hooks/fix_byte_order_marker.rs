...
async fn fix_file(file_base: &Path, filename: &Path) -> Result<HookOutput> {
    ...
    let modified = tokio::task::spawn_blocking(move || -> Result<bool> {
        ...
        if file_len == UTF8_BOM.len() as u64 {
            ...
            shift_file_left(&mut file, file_len, UTF8_BOM.len() as u64)?;
        }
        Ok(true)
    })
    .await??;
...
fn has_utf8_bom(file: &mut fs_err::File) -> Result<bool> {
    ...
    Ok(bom_buffer == UTF8_BOM)
}

fn shift_file_left(file: &mut fs_err::File, file_len: u64, offset: u64) -> Result<()> {
