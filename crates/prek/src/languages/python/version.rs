...
impl PythonRequest {
    ...
    pub(crate) fn satisfied_by(&self, installed_env: &InstalledHookEnv) -> bool {
        ...
        match self {
            ...
            PythonRequest::Path(path) => path == &installed_env.toolchain,
...
mod tests {
    ...
    fn test_satisfied_by() -> anyhow::Result<()> {
        ...
        assert!(PythonRequest::Any.satisfied_by(&installed_env));
        assert!(PythonRequest::Major(3).satisfied_by(&installed_env));
        assert!(PythonRequest::MajorMinor(3, 12).satisfied_by(&installed_env));
        assert!(PythonRequest::MajorMinorPatch(3, 12, 1).satisfied_by(&installed_env));
        assert!(!PythonRequest::MajorMinorPatch(3, 12, 2).satisfied_by(&installed_env));
        assert!(
            PythonRequest::Path(PathBuf::from("/usr/bin/python3.12")).satisfied_by(&installed_env)
        );
        assert!(
            !PythonRequest::Path(PathBuf::from("/usr/bin/python3.11")).satisfied_by(&installed_env)
        );
        ...
        assert!(!PythonRequest::Range(range_req, ">=4.0".to_string()).satisfied_by(&installed_env));
