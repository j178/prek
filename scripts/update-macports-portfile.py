...
def generate_cargo_crates(cargo_lock: Path) -> str:
    with cargo_lock.open("rb") as file:
    ...
    if not crates:
        raise RuntimeError(f"No packages with checksums found in {cargo_lock}")
...
def main() -> None:
    ...
    cargo_crates = generate_cargo_crates(root / "Cargo.lock")
